This document contains the real lab hardware.  It also contains discussion of potential uses.  These are not dictates. Assume the hardware is real, but its uses are up to you to decide.  Note - M5 is presently down.  Rember the current context for our deployment is a lab that support 5 users who are all admins. We are not building an enterpise system but rather a system to support our concepts.
...
Irina serves as the production environment for containerized code execution, LLM model storage, files storage and database 
