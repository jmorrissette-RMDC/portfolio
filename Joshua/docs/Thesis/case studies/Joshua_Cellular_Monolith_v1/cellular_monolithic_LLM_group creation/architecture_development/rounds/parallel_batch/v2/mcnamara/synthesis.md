# McNamara V2 Architecture Specification
...
- **New in this Version:** This V2 specification introduces the Learned Prose-to-Process Mapper (LPPM) as a performance optimization. The LPPM provides a "fast path" for common, simple security queries like "list active alerts" or "acknowledge alert 123," reducing latency for routine SOC operations. All V1 functionalities are preserved.

## 2. Thinking Engine
### 2.1 Imperator Configuration (V1+)
McNamara's Imperator is configured to think like a security analyst. It is trained on security best practices, common attack vectors, and anomaly detection principles.
...
- **LPPM Integration (V2+):** McNamara's LPPM is a distilled transformer model trained on historical conversation logs from V1 deployments. It learns to recognize common request patterns and map them directly to tool call sequences.
  ...
  - **Example Fast Paths:**
    - "List active alerts" → `list_alerts(status='active')`
    - "Show me all alerts" → `list_alerts(status='all')`
    - "Acknowledge alert a1b2-c3d4" → `acknowledge_alert(alert_id='a1b2-c3d4', operator='grace-v2')`
    - "Analyze logs from #logs-turing-v2 for the last hour" → `analyze_logs(conversation_id='#logs-turing-v2', time_window='last 1h')`
  - **Training Loop:**
...
## 3. Action Engine
- **MCP Server Capabilities:** McNamara's MCP server is primarily a consumer of data. It maintains a persistent connection to Rogers, subscribing to all relevant log conversations (e.g., `#logs-*`). It uses an internal rule engine (driven by the Imperator in V2) to process the stream of incoming log events.
- **Tools Exposed:**

```yaml
# Tool definitions for McNamara V2
...
- tool: block_entity
  description: "Temporarily blocks a MAD from accessing resources. (In V2, this creates a HIGH alert recommending a block)."
  parameters:
    - name: mad_identity
      type: string
      required: true
...
- **Container Requirements:**
  ...
  - **Resources:**
    - **CPU:** 0.5 cores
    - **RAM:** 1024 MB (may need to buffer logs for pattern analysis).
- **Configuration:**

| Variable | Description | Example Value |
|---|---|---|
| `JOSHUA_MAD_NAME` | Canonical name of this MAD. | `mcnamara-v2` |
...
| `MCNAMARA_LPPM_MODEL_PATH` | Path to the trained LPPM model file. | `/models/lppm/mcnamara_lppm_v2.onnx` |
...
2.  **Turing:** For each attempt, denies access and sends a `WARN` log to `#logs-turing-v2`.
...
5.  **McNamara:** Calls its own `create_alert` tool with `severity: 'HIGH'`, `title: 'Potential Secret Brute-Force by hopper-v2'`, and details including the target secret and timestamps.
...
5.  **McNamara -> Dewey:** Calls `search_archives(query="mad:hopper-v2", time_window="1h")`.
