...
- **New in this Version:** This V2 specification introduces the Learned Prose-to-Process Mapper (LPPM) as a performance optimization. The LPPM provides a "fast path" for common, high-frequency file operations like "read file" or "list directory," translating them directly into tool calls to reduce latency and cognitive load. All V1 functionalities are preserved.

## 2. Thinking Engine
### 2.1 Imperator Configuration (V1+)
Horace's Imperator is configured to think like a security-conscious system administrator and records manager. It reasons about file permissions, data organization, and access patterns.
...
- **Consulting LLM Usage Patterns:** Horace does not use consulting LLMs in V2. Its operations are highly deterministic.
...
```yaml
# Tool definitions for Horace V2
...
|---|---|---|
| `JOSHUA_MAD_NAME` | Canonical name of this MAD. | `horace-v2` |
...
- **Setup:** Horace's ACLs grant `lovelace-v2` 'READ' access to `/logs/*` but not `/deploy/*`.
...
6.  **Horace's Thinking Engine:** The LPPM/Imperator checks ACLs. The path matches `/deploy/*`, but `lovelace-v2` does not have permission. The request is denied.
7.  **Horace -> Lovelace:** Returns an error: `{ "code": -34402, "message": "ACCESS_DENIED" }`.
8.  **Horace:** Sends a `WARN` level log to its log stream: `"Access denied for 'lovelace-v2' on path '/deploy/config.json'."`
