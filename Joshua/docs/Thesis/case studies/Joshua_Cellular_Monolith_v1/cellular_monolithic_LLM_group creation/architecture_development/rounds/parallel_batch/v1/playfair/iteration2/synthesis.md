# Playfair V1 Architecture Specification
...
- **New in this Version:** This is the initial V1 specification for Playfair. It establishes the foundational cryptographic tools using a software-based key store. This version is strictly V1, with an Imperator that reasons about the appropriate use of cryptographic functions.

## 2. Thinking Engine
- **Imperator Configuration (V1+):** Playfair's Imperator is configured to think like a security engineer specializing in applied cryptography. It understands different cryptographic algorithms, key management policies, and use cases for encryption versus digital signatures.
...
- **Consulting LLM Usage Patterns:** Playfair does not use consulting LLMs in V1. Cryptography must be deterministic and based on proven standards, not probabilistic generation.
...
```yaml
# Tool definitions for Playfair V1
...
# Note: AES-GCM is an authenticated encryption mode. The nonce and authentication tag
# are essential components of the security model. The nonce ensures semantic security
# (same plaintext encrypts to different ciphertext each time), and the tag provides
# integrity verification to detect tampering. These parameters are NOT optional.
...
- **External System Integrations:**
  - **HSM / Secure Key Storage:** In V1, this is a simplification. Playfair will store its keys in a dedicated, encrypted file on its container's secure filesystem, with the master key provided by Turing. A V2+ version would integrate with a true Hardware Security Module (HSM).
...
```json
// Example structure of the DECRYPTED key vault file
{
  "archive_transit_key": {
    "type": "symmetric",
    "versions": {
      "1": "base64-encoded-aes-key..."
    },
    "current_version": 1
  },
...
|---|---|---|
| `JOSHUA_MAD_NAME` | Canonical name of this MAD. | `playfair-v1` |
