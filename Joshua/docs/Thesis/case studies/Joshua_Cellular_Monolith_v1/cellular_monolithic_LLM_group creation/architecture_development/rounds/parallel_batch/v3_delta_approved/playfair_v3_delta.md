# === PLAYFAIR V3 DELTA ===
# Playfair V3 Architecture Specification
...
- **DTR Integration (V3+):** Playfair's DTR is a gradient-boosted decision tree classifier trained on the same data sources as the LPPM. It performs ultra-fast classification-based routing for the most common request patterns.
  ...
  - **Example Ultra-Fast Paths:**
    - "Encrypt 'my secret message' using AES-256"
    - "Decrypt '[ciphertext]' with key '[key]'"
    - "Generate a new 2048-bit RSA key pair"
    - "Hash 'password123' using SHA-256"
...
|---|---|---|
| `PLAYFAIR_DTR_MODEL_PATH` | Path to the trained DTR model file. | `/models/dtr/playfair_dtr_v3.onnx` |
...
- **Workflow: Hashing data**
  1. Request: "sha256 hash 'hello world'"
  2. Playfair's DTR receives the request.
  3. DTR classifies the request as `hash_data` with 100% confidence, identifying 'sha256' and 'hello world'.
  4. DTR bypasses LPPM/Imperator and calls the hashing library directly.
  5. The resulting hash is returned in <10ms.
