...
**Architecture:**
*   **Model:** A sophisticated neural network, specifically a transformer-based model trained as a relevance ranker or context selector. It is not a simple retrieval system; it is an ML component that learns what context is most relevant for a given reasoning task.
...
**Training Pipeline:**
The CET model is continuously improved through a feedback loop:
1.  **Data Collection:** After every Imperator task, the full interaction is logged to Dewey: the original request, the context package provided by the CET, the Imperator's reasoning and action, and the final outcome (success/failure, user feedback).
2.  **Training:** The model is periodically retrained using this data. Successful outcomes reinforce the relevance scores of the context chunks that were provided. Failed outcomes penalize the scores of the provided context, teaching the model to select better information next time.
3.  **Validation:** The goal is a measurable improvement in Imperator task success rate. The target is a **20-40% improvement** in first-time task completion for complex requests compared to using a simple, fixed-window context.
...
*   **Dewey:** **(Expanded Usage)** Required for DTR training, LPPM training, AND as a critical real-time context source for the CET via `search_archives`.
...
*   **Resources:**
    *   **CPU:** 0.75 - 1.0 cores (Increased to support CET context assembly)
    *   **RAM:** 768 MB - 1 GB (Increased to hold CET model in memory)
...
| `TURING_CET_MODEL_PATH`            | **(V4)** Path to the trained CET model checkpoint.                          | `/models/turing/cet/v1.0.pt`                   |
| `TURING_CET_CONTEXT_LIMIT`         | **(V4)** Maximum number of tokens for the assembled context package.        | `2048`                                         |
| `TURING_CET_SEARCH_DEPTH`          | **(V4)** How many days back in Dewey archives to search for context.        | `90`                                           |
