...
- **New in this Version:** This V2 specification introduces the Learned Prose-to-Process Mapper (LPPM) as a performance optimization. The LPPM recognizes common, repetitive administrative commands and maps them directly to tool calls, bypassing the full reasoning loop of the Imperator for significant speed gains. All V1 functionalities are preserved.

## 2. Thinking Engine
### 2.1 Imperator Configuration (V1+)
Rogers' Imperator is configured for a very narrow, administrative role. It does not engage in creative reasoning but instead interprets administrative commands related to conversation management.
...
- **Consulting LLM Usage Patterns:** Rogers does not use consulting LLMs. Its domain is too narrow and deterministic.
...
```yaml
# Tool definitions for Rogers V2
...
|---|---|---|
| `JOSHUA_MAD_NAME` | Canonical name of this MAD. | `rogers-v2` |
...
1.  **Grace -> Rogers:** Sends a `create_conversation` request with `participants=['grace-v2', 'hopper-v2']`, `name='script-writing-task'`.
...
1.  **Hopper -> Rogers:** Sends `send_message` to `conv-abc-123`: `"Grace, can you add Horace to this conversation? I need file access."`
2.  **Grace -> Rogers:** Sends a request to a (hypothetical V2.1) `add_participant` tool with `conversation_id='conv-abc-123'`, `mad_identity='horace-v2'`.
3.  **Rogers:** Adds Horace to the participant list for the conversation.
4.  **Horace:** Now receives all new messages in `conv-abc-123`.
5.  **Hopper -> Rogers:** Sends `send_message`: `"Horace, I need read access to /logs/system.log."` Horace receives the message and can now act.
