# === DEWEY V3 DELTA ===
# Dewey V3 Architecture Specification
...
- **DTR Integration (V3+):** Dewey's DTR is a gradient-boosted decision tree classifier trained on the same data sources as the LPPM. It performs ultra-fast classification-based routing for the most common request patterns.
  ...
  - **Example Ultra-Fast Paths:**
    - "Save conversation conv_id_123"
    - "Retrieve message msg_id_456"
    - "Delete conversation conv_id_789"
    - "Find all conversations for user_abc"
    - "Update message msg_id_101 with new content"
  - **Training Loop:**
...
|---|---|---|
| `DEWEY_DTR_MODEL_PATH` | Path to the trained DTR model file. | `/models/dtr/dewey_dtr_v3.onnx` |
...
- **Workflow: Retrieving a conversation**
  1. Request: "Get conversation 998877"
  2. Dewey's DTR receives the request.
  3. DTR classifies the request as `retrieve_conversation_by_id` with 99.8% confidence.
  4. DTR bypasses LPPM/Imperator and calls the database query tool with the ID `998877`.
  5. The conversation data is returned in <10ms.
