# === ROGERS V3 DELTA ===
# Rogers V3 Architecture Specification
...
- **DTR Integration (V3+):** Rogers's DTR is a gradient-boosted decision tree classifier trained on the same data sources as the LPPM. It performs ultra-fast classification-based routing for the most common request patterns.
  ...
  - **Example Ultra-Fast Paths:**
    - "Get conversation history for user_123"
    - "Send message 'Hello' to channel #general"
    - "Is user_456 online?"
    - "Fetch last 10 messages from conversation_abc"
    - "Create a new conversation with user_789"
  - **Training Loop:**
...
|---|---|---|
| `ROGERS_DTR_MODEL_PATH` | Path to the trained DTR model file. | `/models/dtr/rogers_dtr_v3.onnx` |
...
- **Workflow: Fetching recent messages**
  1. Request: "Get my last 5 messages"
  2. Rogers' DTR receives the request.
  3. DTR classifies the request as `get_recent_messages` with 99% confidence.
  4. DTR bypasses LPPM and Imperator, directly calling the tool to query Dewey (Conversation Storage).
  5. The message list is returned to the user in <10ms.
