# Fiedler V2 Architecture Specification
...
- **New in this Version:** This V2 specification introduces the Learned Prose-to-Process Mapper (LPPM) as a performance optimization. The LPPM recognizes common, simple requests like "list models" or "use gpt-4o" and maps them directly to tool calls, bypassing the Imperator's reasoning for faster, more deterministic responses. All V1 functionalities are preserved.

## 2. Thinking Engine
### 2.1 Imperator Configuration (V1+)
Fiedler's Imperator is configured to think like an expert on AI models and resource management. It reasons about the strengths, weaknesses, costs, and appropriate use cases for different LLMs.
...
- **LPPM Integration (V2+):** Fiedler's LPPM is a distilled transformer model trained on historical conversation logs from V1 deployments. It learns to recognize common request patterns and map them directly to tool call sequences.
  ...
  - **Example Fast Paths:**
    ...
    - "What is the current config?" → `get_config()`
    - "Use gpt-4o to summarize this text: ..." → `send(model_alias='gpt-4o', prompt='summarize this text: ...')`
    - "Which API keys are set?" → `list_keys()`
  - **Training Loop:**
...
```yaml
# Tool definitions for Fiedler V2
...
|---|---|---|
| `JOSHUA_MAD_NAME` | Canonical name of this MAD. | `fiedler-v2` |
...
| `FIEDLER_LPPM_MODEL_PATH` | Path to the trained LPPM model file. | `/models/lppm/fiedler_lppm_v2.onnx` |
