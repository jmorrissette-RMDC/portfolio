# === MCNAMARA V3 DELTA ===
# McNamara V3 Architecture Specification
...
- **DTR Integration (V3+):** McNamara's DTR is a gradient-boosted decision tree classifier trained on the same data sources as the LPPM. It performs ultra-fast classification-based routing for the most common request patterns.
  ...
  - **Example Ultra-Fast Paths:**
    - "Check health of service 'Rogers'"
    - "Get CPU usage for 'Fiedler' over the last hour"
    - "List all active alerts"
    - "What is the memory consumption of 'Dewey'?"
    - "Show me the error rate for 'Sergey'"
  - **Training Loop:**
...
|---|---|---|
| `MCNAMARA_DTR_MODEL_PATH` | Path to the trained DTR model file. | `/models/dtr/mcnamara_dtr_v3.onnx` |
...
- **Workflow: Simple health check**
  1. Request: "Is Grace healthy?"
  2. McNamara's DTR receives the request.
  3. DTR classifies the request as `service_health_check` with 99.6% confidence, identifying 'Grace'.
  4. DTR bypasses LPPM/Imperator and calls the monitoring backend for the health status of the Grace service.
  5. The health status ('Healthy' or 'Unhealthy') is returned in <10ms.
