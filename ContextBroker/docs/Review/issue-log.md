...
| TA-04 | Test | Opus | major | Extraction worker has 42% error rate on fresh deployments (52 errors out of 124 attempts). Root cause: Mem0 is not functional (TA-02), so every extraction call fails. The worker retries each conversation 3 times then marks messages as extracted to stop retrying — but the extraction never actually succeeded. Messages are marked as processed without being processed. | `app/workers/db_worker.py`, `packages/context-broker-ae/src/context_broker_ae/memory_extraction.py` | OPEN | Dependent on TA-02. Once Mem0 initializes at startup, extraction will work. However, the worker should also distinguish between "Mem0 not available" (retry later) and "extraction failed for this content" (mark and move on). Currently it treats both the same way. |
