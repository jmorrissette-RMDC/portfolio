...
2. **Ingestion:** A client calls `conv_store_message`. The system deduplicates the message (using a client-supplied idempotency key) and persists it to PostgreSQL. It enqueues background jobs to Redis and returns success immediately.
...
- **Search:** Hybrid vector (pgvector) + full-text search (`ts_rank`, PostgreSQL's implementation of BM25-style ranking), combined via Reciprocal Rank Fusion (RRF), with optional cross-encoder reranking. (Note: The `conv_search` tool queries conversation-level metadata and summaries, whereas `conv_search_messages` targets individual verbatim messages.)
