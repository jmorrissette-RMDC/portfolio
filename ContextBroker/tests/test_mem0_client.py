...
def _reset_mem0_singleton():
    """Ensure clean singleton state for every test in this module."""
...
class TestNeo4jConfig:
    ...
    @patch.dict(
        os.environ, {"NEO4J_HOST": "host-with-special.chars", "NEO4J_PORT": "7687"}
    )
...
class TestGetEmbeddingDims:
    def test_explicit_config_wins(self):
        assert (
            _get_embedding_dims(
                {"embedding_dims": 256}, {"model": "text-embedding-3-small"}
            )
            == 256
        )
    ...
    def test_missing_dims_raises(self):
        """embedding_dims is required — no hardcoded fallback."""
        with pytest.raises(ValueError, match="embedding_dims is required"):
            _get_embedding_dims({}, {"model": "text-embedding-3-small"})

    def test_missing_dims_no_model_raises(self):
        with pytest.raises(ValueError, match="embedding_dims is required"):
            _get_embedding_dims({}, {})

    def test_explicit_none_raises(self):
        with pytest.raises(ValueError, match="embedding_dims is required"):
            _get_embedding_dims({"embedding_dims": None}, {"model": "nomic-embed-text"})
...
class TestGetMem0Client:
    ...
    async def test_returns_none_on_init_failure(self):
        ...
        with patch.object(
            mod, "_build_mem0_instance", side_effect=ConnectionError("neo4j down")
        ):
    ...
    async def test_caches_instance(self):
        ...
        with patch.object(
            mod, "_build_mem0_instance", return_value=mock_instance
        ) as mock_build:
...
class TestBuildMem0Instance:
    ...
    def test_passes_real_neo4j_password(self, mock_memory):
        ...
        pwd = (
            gs_config["password"] if isinstance(gs_config, dict) else gs_config.password
        )
        assert pwd == "real-pw"
    ...
    def test_dummy_neo4j_password_when_auth_none(self, mock_memory):
        ...
        usr = (
            gs_config["username"] if isinstance(gs_config, dict) else gs_config.username
        )
        assert pwd == "neo4j"
        assert usr == "neo4j"
    ...
    def test_uses_extraction_config_not_llm(self, mock_memory):
        ...
        config = {
            "extraction": {
                "base_url": "http://extraction-host/v1",
                "model": "extraction-model",
                "api_key_env": "",
            },
            "llm": {
                "base_url": "http://wrong-host/v1",
                "model": "wrong-model",
                "api_key_env": "",
            },
            "embeddings": {
