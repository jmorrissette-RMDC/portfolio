...
def test_a1():
    ...
    log = ssh_cmd(
        "docker logs context-broker-langgraph 2>&1 | grep 'Registered AE package' | tail -1"
    )
...
def test_a2():
    ...
    log = ssh_cmd(
        "docker logs context-broker-langgraph 2>&1 | grep 'Registered TE package' | tail -1"
    )
...
def test_a3():
    ...
    result = mcp_call(
        "install_stategraph", {"package_name": "context-broker-te"}, timeout=120
    )
...
def test_a5():
    ...
    assert len(content) > 0, "Empty response from Imperator"
...
def test_b1():
    ...
    output = ssh_cmd(
        "docker exec context-broker-langgraph cat /config/credentials/.env | grep -c '='"
    )
...
def test_b3():
    ...
    conv = mcp_call(
        ...
        {"title": "b3-test", "flow_id": "test", "user_id": "test"},
    )
...
def test_b4():
    ...
    assert (
        int(count or "0") > 0
    ), "No embeddings generated — embedding provider may be misconfigured"
...
def test_c1():
    ...
    conv = mcp_call(
        "conv_create_conversation",
        {"title": "c1-pipeline", "flow_id": "test", "user_id": "test"},
    )
    conv_id = conv["conversation_id"]
    for i in range(5):
        mcp_call(
...
def test_c3():
    ...
    assert result.get("total_tokens", 0) > 0, "Zero tokens in context"
...
def test_c4():
    ...
    assert len(messages) > 0, "Search returned no results"
...
def test_d3():
    ...
    result = chat_call(
        "What is the current pipeline status? Use your pipeline status tool.",
        timeout=180,
    )
    content = result["choices"][0]["message"]["content"]
...
def test_d4():
    ...
    result = chat_call(
        "Show me the last 5 log entries from any container. Use your log query tool.",
        timeout=180,
    )
    content = result["choices"][0]["message"]["content"]
...
def test_d5():
    ...
    result = chat_call(
        "Introspect your own context. What build type and tiers are you using?",
        timeout=180,
    )
    content = result["choices"][0]["message"]["content"]
...
def test_e3():
    ...
    if not conv_id:
        results.append(
            TestResult("E3: Effective utilization", False, "No conversation to test")
        )
        return
    result = mcp_call(
        "get_context",
...
def test_f2():
    ...
    conv = mcp_call(
        "conv_create_conversation",
        {"title": "f2-resilience", "flow_id": "test", "user_id": "test"},
    )
    result = mcp_call(
        "store_message",
        {
...
def test_g2():
    ...
    assert (
        int(count or "0") > 0
    ), "No logs in system_logs table — log shipper may not be working"
...
def test_g3():
    ...
    log_line = ssh_cmd(
        "docker logs context-broker-langgraph 2>&1 | grep '{\"timestamp' | tail -1"
    )
